...
class Literal(Expr):
    ...
    def eval(self, psstacks):
        ...
        if isinstance(self.value,int):
            psstacks.opPush(int(self.value))
        elif isinstance(self.value, bool):
            psstacks.opPush(bool(self.value))
...
class Name(Expr):
    ...
    def eval(self,psstacks):
        ...
        if self.var_name[0] == '/':
            ...
            vals = psstacks.lookup(self.var_name)
            
            if isinstance(vals, CodeArray) and psstacks.scope == "static":
                #find def of val
                def findStatic(name):
                    newN = '/' + name
                    top = len(psstacks.dictstack) -1
                    if newN in psstacks.dictstack[top][1]:
                        return top
                    else:
                        link = psstacks.dictstack[top][0]
                        while link != 0:
                            if newN in psstacks.dictstack[top][1]:
                                return top
                            else:
                                link = psstacks.dictstack[link][0]
                        if newN in psstacks.dictstack[link][1]:
                            return 0
                        else:
                            print("Error")
                
                if psstacks.lookup(self.var_name) is not None:
                    index = findStatic(self.var_name)
                    tuples = (index, {})
                    psstacks.dictPush(tuples)
                    vals.apply(psstacks)
                    psstacks.dictPop()
            elif isinstance(vals, CodeArray) and psstacks.scope == "dynamic":
                if isinstance(vals, CodeArray):
                    vals.apply(psstacks)
                else:
                    psstacks.opPush(vals)
